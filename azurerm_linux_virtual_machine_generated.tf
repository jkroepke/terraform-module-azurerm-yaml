resource "azurerm_network_interface" "azurerm_linux_virtual_machine" {
  for_each = local.linux_virtual_machines_network_interface
  ...
  dynamic "ip_configuration" {
    ...
    content {
      ...
      public_ip_address_id = (contains(keys(ip_configuration.value), "public_ip_address_id")
        ? (contains(keys(azurerm_public_ip.this), ip_configuration.value.public_ip_address_id)
...
resource "azurerm_linux_virtual_machine" "this" {
  ...
  network_interface_ids = concat(
    [for nic, _ in local.linux_virtual_machines_network_interface : azurerm_network_interface.azurerm_linux_virtual_machine[nic].id],
  ...
  dynamic "admin_ssh_key" {
    for_each = contains(keys(each.value), "admin_ssh_key") ? each.value.admin_ssh_key : {}
    content {
