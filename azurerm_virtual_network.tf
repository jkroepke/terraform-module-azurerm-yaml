...
resource "azurerm_subnet_route_table_association" "this" {
  for_each = {
    ...
    "${subnet.virtual_network_name}|${subnet.name}|${try(subnet.route_table_id, subnet.route_table)}" => subnet
    if try(subnet.route_table_id, subnet.route_table, null) != null
  }

  route_table_id = try(each.value.route_table_id, contains(keys(azurerm_resource_group.this), each.value.route_table) ? azurerm_route_table.this[each.value.route_table] : data.azurerm_route_table.subnet_route_table_association[each.key])
...
data "azurerm_route_table" "subnet_route_table_association" {
  for_each = {
    for subnet in flatten(local.virtual_networks_subnets) : "${subnet.virtual_network_name}|${subnet.name}|${subnet.route_table}" => subnet
    if (can(subnet.route_table) && !contains(keys(azurerm_route_table.this), try(subnet.route_table, "")))
  }
